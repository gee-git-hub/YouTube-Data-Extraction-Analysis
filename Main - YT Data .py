...
mydb = sql.connect(host="localhost",user="root",password="root")
...
mydb = sql.connect(host="localhost",user="root",password="root",database="world")
mycursor = mydb.cursor()
...
# HOME PAGE
if selected == "☰ Home":
    #st.markdown("## YouTube Data Harvesting and Warehousing using SQL and Streamlit")
    st.header(':red[YouTube] Data Harvesting', divider='grey')
    st.image('yt log.gif')
    st.toast('Created by Geethapriya', icon='🤖')
...
if selected == "📉 Query":
    ...
    index=None,
    placeholder="Select One Analysis Question",
    )
